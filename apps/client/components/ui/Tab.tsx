...
export default function Tabs() {
  ...
  return (
    <div className="container-wide w-full mx-auto py-6">
      <div className="flex items-center justify-start md:justify-center gap-x-10 overflow-x-auto no-scrollbar w-full">
        {categories.map((tab) => (
          <button
            key={tab}
            onClick={() => setActiveTab(tab)}
            className={`relative px-1 pb-4 text-[15px] font-bold transition-all duration-300 whitespace-nowrap flex-shrink-0
                  ${
                    activeTab === tab
