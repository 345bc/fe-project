...
function UpdateCategoryPageContent() {
  ...
  useEffect(() => {
    ...
    const fetchCategory = async () => {
      try {
        ...
        setName(c.name || "");
        setIntroduce(c.introduce || "");
        setImage(c.image || "");
      } catch (err: any) {
        setError(err?.message || "Lỗi tải thông tin danh mục");
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      await categoryService.patchCategory(catId, {
        name,
        introduce,
        image,
      });
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-xl font-semibold text-gray-900">Cập nhật danh mục Tour</h1>
        </div>
        <button
